        ...
        <section id="status" class="services sections-bg">
            <div class="container" data-aos="fade-up">
                ...
                <div class="section-header">
                    <h2>Update Developer's CV</h2>

                </div>
                <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post" enctype="multipart/form-data">
                    <div class="form-group" style="padding-left: 20%;padding-right: 20%; width: 100%;">
                        <input type="email" class="form-control" name="email_CV" placeholder="Enter Developers's Email Address*" required style="height: 40px;border-radius: 10px;font-size: 15px;" />
                        <div class="validation"></div>
                    </div>
                    <div style="width: 100%;padding-left: 20%;padding-right: 20%; display: inline-flex;margin-top: 15px;">
...
if (isset($_POST['submit'])) {
    ...
    $query = "Update Developers_info Set CV='$fileName' where EmailAddress='$email'";
    $run = mysqli_query($con, $query);

    if ($run) {
        move_uploaded_file($fileTmpName, $path);
        $_SESSION['status'] = "data inserted successfully";
        echo '<script>alert("Developer\'s cv has been updated!")</script>';
    } else {
